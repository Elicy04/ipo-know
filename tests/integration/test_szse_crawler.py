...
调用 SZSEIPOCrawler 采集汽车制造业有效文件清单,
将结果以 JSON 格式保存到 tests/integration/output/ 目录.

用法:
    # 不使用代理
    python tests/integration/test_szse_crawler.py

    # 使用代理
    python tests/integration/test_szse_crawler.py --proxy http://127.0.0.1:7890
"""
...
from ipo_know.crawler.szse_ipo_crawler import SZSEIPOCrawler  # noqa: E402
...
def main() -> None:
    """执行深交所爬虫集成测试."""
    parser = argparse.ArgumentParser(description='深交所爬虫集成测试')
    parser.add_argument(
    ...
    logger.info('=' * 60)
    logger.info('深交所爬虫集成测试开始')
    logger.info('=' * 60)

    crawler = SZSEIPOCrawler()

    # ---- 采集有效文件清单 ----
    print(f'\n采集有效文件: industry={TEST_INDUSTRY}')
    try:
        files = crawler.collect(industry=TEST_INDUSTRY)
    except Exception as exc:
        logger.exception('深交所文件采集失败')
        print(f'\n[FAILED] 深交所文件采集失败: {exc}')
        return

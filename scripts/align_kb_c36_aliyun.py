"""临时脚本: 一键对齐 C36 有效文件清单与阿里云百炼知识库.

流程:
    1. 调用 crawler 采集汽车制造业(C36)、主板+科创板、全审核状态
       的有效披露文件清单;
    2. 调用 kb_align 将清单与阿里云百炼知识库做全量对齐:
       清单有而知识库缺失的文档下载后经租约上传、解析、入索引,
       并为数据中心文件打 fileid_ 锚点等标签供后续反向映射;
       知识库有而不在清单内的文档从索引与数据中心一并删除.

用法:
    uv run python align_kb_c36_aliyun.py            # 实际执行增删
    uv run python align_kb_c36_aliyun.py --dry-run  # 只打印差异不执行
"""
...
async def main() -> None:
    ...
    if not dry_run:
        ...
        if report.failed_adds:
            ...
            for file_id, reason in report.failed_adds:
                print(f'  - {file_id}: {reason}')
        if report.failed_deletes:
            print(f'删除失败: {len(report.failed_deletes)} 篇')
            for aliyun_id, reason in report.failed_deletes:
                print(f'  - {aliyun_id}: {reason}')
    print('=' * 62)

...
保存状态, 请求时全量回传), 回答经 markdown 气泡流式渲染
...
class ChatPanel:
    ...
    Attributes:
        ...
        _current_bubble: 当前 AI 回答气泡.
        _current_md: 气泡内 markdown 组件引用.
    ...
    def __init__(
        ...
        self._current_bubble: ui.chat_message | None = None
    ...
    def _build_ui(self) -> None:
        ...
        with container:
            with ui.row().classes(
                ...
                ui.label('知识问答').classes('text-lg font-bold')
                ui.button(
                    '清空对话',
            ...
            with ui.row().classes('w-full items-end gap-2'):
                self._query_input = ui.textarea(
                    label='提问内容',
                    placeholder=(
                        f'输入问题 (不超过 {_QUERY_MAX_LENGTH} 字)'
                    ),
                ).classes('flex-1').props('autogrow rows=2')
    ...
    def _prepare_bubbles(self, query: str) -> None:
        """渲染用户消息气泡并创建空的 AI 回答气泡.

        AI 气泡内嵌单个 markdown 组件持有引用, 供流式
        flush 原地更新.
        ...
        with self._messages_col:
            ui.chat_message(text=query, sent=True)
            bubble = ui.chat_message(sent=False)
            with bubble:
    ...
    def _ensure_thinking_area(self) -> None:
        ...
        首个 reasoning_delta 到达时在当前 AI 气泡内、正文
        markdown 之上创建: ``ui.expansion`` 构造参数
    ...
    def _degrade_thinking_to_plain_text(self) -> None:
        ...
        with expansion:
            label = ui.label(self._thinking_buffer).classes(
                'w-full whitespace-pre-wrap text-sm text-gray-400'
            )
    ...
    def _render_references(self, hits: list[SearchHit]) -> None:
        ...
        label 按展示上限追加), 不新建多个. 槽位恒为 0:
        首次创建 move 到气泡最前, 若思考区已存在则思考区
        后移让位, 保证 DOM 顺序恒为 引用 → 思考 → 正文.
    ...
    def _append_reference_labels(
        ...
        """在引用折叠区内追加本批切片的展示 label.

        展示总数受 ``_REFERENCES_LIMIT`` 钳制, 超额部分仅
        计入标题累计条数不渲染.
    ...
    def _finish_bubble(self, stopped: bool) -> None:
        ...
        if stamp and self._current_bubble is not None:
            # q-chat-message 的 stamp 无构造后公开 setter,
            # 直接写 props 并推送更新 (NiceGUI 惯用手法)
            self._current_bubble._props['stamp'] = stamp
    ...
    def _refresh_btn_state(self) -> None:
        ...
        能力位取 ``_chat_supported`` 缓存 (set_platform 时
        按后端 ``supports_chat`` 刷新), 避免每次 tick 构造
        backend.
        """
        if self._btn_send is not None:

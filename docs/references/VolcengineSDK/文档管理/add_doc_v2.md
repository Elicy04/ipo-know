...
| | | | | | \
|project_name |Optional[str] |否 |default |知识库所属项目，获取方式参考文档 [API 接入与技术支持](https://www.volcengine.com/docs/84313/1606319?lang=zh#1ab381b9) |\
...
| | | | | |\
| | | | |* 只能使用英文字母、数字、下划线_，并以英文字母或下划线开头，不能为空 |\
| | | | |* 长度要求：[1, 128] |
| | | | | | \
|doc_name |Optional[str] |否 |-- |**文档名称** |\
| | | | | |\
| | | | |* 对于 tos 导入的方式，未传入时直接使用 tos path 下面的文档名 |\
| | | | |* 对于 url 导入的方式，先通过 url 提取带后缀的文档名，如果没有则返回错误码 400，要求用户再传 doc_name |\
| | | | | |\
| | | | |格式要求： |\
| | | | | |\
| | | | |* 不能包含有特殊用途的字符 < > : ' " / \ | ? * 及空格 |\
...
| | | | | |\
| | | | |* 非结构化文档支持类型：txt, doc, docx, pdf, markdown, pptx, ppt, jpeg, png, webp, bmp, mp4, mp3, wav, aac, flac, ogg |\
| | | | |* .jpg 和 .jpeg 文件 doc_type 均为 jpeg |\
| | | | |* .markdown 和 .md 文件 doc_type 均为 markdown |\
| | | | |* 结构化文档支持类型：xlsx, csv, jsonl |\
| | | | | |\
| | | | |优先使用传入的值；若未传入，将尝试自动提取；若自动提取失败，则接口返回错误 |
| | | | | | \
|description |Optional[str] |否 |-- |**文档描述** |\
| | | | |描述会参与对图片的检索，如电商场景下，描述可以用于存放图片对应的详细商品说明，售卖亮点，价格等 |\
| | | | |注： |\
| | | | | |\
| | | | |* 暂**只在** doc_type 为**图片类型文档时支持**，其他类型文档设置无效。 |\
| | | | |* 长度 [0，4000] |
| | | | | | \
|tag_list |Optional[List[MetaItem]] |否 |-- |Tag 为结构体，包含： |\
| | | | | |\
| | | | |* field_name：标签名，类型为 string |\
| | | | |* 不能为 "doc_id" |\
| | | | |* 需对齐创建知识库时的 field_name |\
| | | | |* 在创建知识库时先初始化标签索引，再在上传文档时打标，以用于检索时实现标签过滤能力 |\
| | | | |* 若需新增过滤标签，请先编辑知识库新增标签后，再进行文档打标 |\
| | | | |* field_type：标签类型 |\
| | | | |* 支持 "int64"、"float32"、"string"、"bool"、"list"、"date_time"、"geo_point" 类型 |\
| | | | |* 需对齐创建知识库时的 field_type |\
| | | | |* field_value：标签值 |\
| | | | |* 与 field_type 指定类型一致 |
| | | | | | \
...
| | | | |* tos://a/b/c.pdf |
| | | | | | \
|path_segments |Optional[List[str]] |否 |-- |文档所在的目录路径，列表元素按目录层级顺序填写。例如：`["产品文档", "V2.0", "使用手册"]` |\
| | | | | | \
|strategy_resource_id |Optional[str] |否 |-- |**切片策略资源 id** |\
| | | | |指定文档上传时使用的切片策略，如 `kb-strategy-xxxx`。不传时使用知识库默认切片策略 |\
| | | | |注：官方在线文档暂未列出该参数，但 SDK (vikingdb-python-sdk) 的 AddDocV2Request 模型中存在该字段，且会随请求体一并提交 |
...
<span id="adddocresponsedata"></span>
### **AddDocResponseData**

官方原文文档链接：https://mineru.net/apiManage/docs
以下是为了方便阅读，对原文html转换为Markdown格式的结果(20260812采集)：
---
# MinerU 文档解析接口文档
...
接口为异步返回模式，提交成功后返回task_id，需通过查询接口轮询结果。
...
- 调用本接口，传入文件名等参数，获取task_id、OSS 签名上传 URL（file_url）
- 客户端使用PUT 方法将文件直接上传到 file_url
- 上传完成后，后端自动检测并开始解析
- 通过查询接口轮询解析结果
...
通过task_id 查询解析任务的状态和结果。任务处理完成后，响应中包含 Markdown 结果文件的 CDN 下载链接。
